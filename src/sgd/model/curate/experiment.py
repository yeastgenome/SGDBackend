...
class ExperimentAlias(Base, EqualityByIDMixin, UpdateWithJsonMixin, ToJsonMixin):
    ...
    def unique_key(self):
        return (None if self.experiment is None else self.experiment.unique_key()), self.display_name, self.alias_type
    ...
    def create_or_find(cls, obj_json, session, parent_obj=None):
        ...
        if parent_obj is not None:
            newly_created_object.experiment_id = parent_obj.id

        current_obj = session.query(cls)\
            .filter_by(experiment_id=newly_created_object.experiment_id)\
...
class ExperimentRelation(Base, EqualityByIDMixin, UpdateWithJsonMixin, ToJsonMixin):
    ...
    #Relationships
    parent = relationship(Experiment, backref=backref("children", cascade="all, delete-orphan", passive_deletes=True), uselist=False, foreign_keys=[parent_id])
    child = relationship(Experiment, backref=backref("parents", cascade="all, delete-orphan", passive_deletes=True), uselist=False, foreign_keys=[child_id])
    source = relationship(Source, uselist=False)

    __eq_values__ = ['id', 'relation_type', 'date_created', 'created_by']
    __eq_fks__ = [('source', Source, False),
                  ('parent', Experiment, False),
    ...
    def create_or_find(cls, obj_json, session, parent_obj=None):
        ...
        child, status = Experiment.create_or_find(obj_json, session)
        #if status == 'Created':
