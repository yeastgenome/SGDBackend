...
class Qualifier(Base, EqualityByIDMixin, ToJsonMixin, UpdateWithJsonMixin):
    __tablename__ = 'qualifier'

    id = Column('qualifier_id', Integer, primary_key=True)
    ...
    __eq_fks__ = [('source', Source, False),
                  ('aliases', 'qualifier.QualifierAlias', True),
...
class QualifierAlias(Base, EqualityByIDMixin, UpdateWithJsonMixin, ToJsonMixin):
    __tablename__ = 'qualifier_alias'
    ...
    source_id = Column('source_id', Integer, ForeignKey(Source.id))
    bud_id = Column('bud_id', Integer)
    qualifier_id = Column('qualifier_id', Integer, ForeignKey(Qualifier.id, ondelete='CASCADE'))
    alias_type = Column('alias_type', String)
    date_created = Column('date_created', Date, server_default=FetchedValue())
    created_by = Column('created_by', String, server_default=FetchedValue())

    #Relationships
    qualifier = relationship(Qualifier, uselist=False, backref=backref('aliases', cascade="all, delete-orphan", passive_deletes=True))
    source = relationship(Source, uselist=False)
    ...
    def unique_key(self):
        return (None if self.qualifier is None else self.qualifier.unique_key()), self.display_name, self.alias_type
    ...
    def create_or_find(cls, obj_json, session, parent_obj=None):
        ...
        if parent_obj is not None:
            newly_created_object.qualifier_id = parent_obj.id

        current_obj = session.query(cls)\
            .filter_by(qualifier_id=newly_created_object.qualifier_id)\
...
class QualifierRelation(Base, EqualityByIDMixin, UpdateWithJsonMixin, ToJsonMixin):
    __tablename__ = 'qualifier_relation'

    id = Column('relation_id', Integer, primary_key=True)
    source_id = Column('source_id', Integer, ForeignKey(Source.id))
    parent_id = Column('parent_id', Integer, ForeignKey(Qualifier.id, ondelete='CASCADE'))
    child_id = Column('child_id', Integer, ForeignKey(Qualifier.id, ondelete='CASCADE'))
    relation_type = Column('relation_type', String)
    date_created = Column('date_created', Date, server_default=FetchedValue())
    created_by = Column('created_by', String, server_default=FetchedValue())

    #Relationships
    parent = relationship(Qualifier, backref=backref("children", cascade="all, delete-orphan", passive_deletes=True), uselist=False, foreign_keys=[parent_id])
    child = relationship(Qualifier, backref=backref("parents", cascade="all, delete-orphan", passive_deletes=True), uselist=False, foreign_keys=[child_id])
    source = relationship(Source, uselist=False)

    __eq_values__ = ['id', 'relation_type', 'date_created', 'created_by']
    __eq_fks__ = [('source', Source, False),
                  ('parent', Qualifier, False),
    ...
    def create_or_find(cls, obj_json, session, parent_obj=None):
        ...
        child, status = Qualifier.create_or_find(obj_json, session)
        #if status == 'Created':
