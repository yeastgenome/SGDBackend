...
class Goterm(Base, EqualityByIDMixin, ToJsonMixin, UpdateWithJsonMixin):
    ...
    id = Column('goterm_id', String, primary_key=True)
...
class GotermUrl(Base, EqualityByIDMixin, UpdateWithJsonMixin, ToJsonMixin):
    ...
    source_id = Column('source_id', String, ForeignKey(Source.id))
    bud_id = Column('bud_id', Integer)
    goterm_id = Column('goterm_id', String, ForeignKey(Goterm.id, ondelete='CASCADE'))
    ...
    def unique_key(self):
        return (None if self.goterm is None else self.goterm.unique_key()), self.name, self.link
...
class GotermAlias(Base, EqualityByIDMixin, UpdateWithJsonMixin, ToJsonMixin):
    ...
    def unique_key(self):
        return (None if self.goterm is None else self.goterm.unique_key()), self.name, self.alias_type
