...
class ArchDnasubsequence(Base, EqualityByIDMixin, ToJsonMixin, UpdateWithJsonMixin):
    __tablename__ = 'arch_dnasubsequence'

    id = Column('dnasubsequence_id', Integer, primary_key=True)
    annotation_id = Column('annotation_id', Integer, ForeignKey(ArchDnaseqannotation.id))
    ...
    __eq_values__ = ['id', 'annotation_id', 'dbentity_id', 'display_name', 'bud_id', 
                     ...
                     'residues', 'date_created', 'created_by', 'date_archived']
    # ('file', File, False),
    __eq_fks__ = [('so', So, False),
                  ('dbentity', Dbentity, True),
                  ('genomerelease', Genomerelease, False)]
    __id_values__ = ['id']
    __no_edit_values__ = ['id', 'date_created', 'created_by', 'date_archived']
