...
CONSTRAINT EXPRESSIONEVIDENCE_UK UNIQUE (GEO_ID, PCL_FILENAME, CONDITION)) TABLESPACE DATA2;
...
DROP TABLE EXPRESSIONDATA CASCADE CONSTRAINTS;
CREATE TABLE EXPRESSIONDATA (
EXPRESSIONDATA_ID INTEGER NOT NULL,
EVIDENCE_ID INTEGER NOT NULL,
BIOENTITY_ID INTEGER NOT NULL,
VALUE DECIMAL(7, 3) NOT NULL,
CONSTRAINT EXPRESSIONDATA_PK PRIMARY KEY (EXPRESSIONDATA_ID),
CONSTRAINT EXPRESSIONDATA_UK UNIQUE (EVIDENCE_ID, BIOENTITY_ID)) TABLESPACE DATA2;
CREATE INDEX EXPDAT_BIOENT_FK_INDEX ON EXPRESSIONDATA (BIOENTITY_ID) TABLESPACE DATA2;
CREATE INDEX EXPDAT_EV_FK_INDEX ON EXPRESSIONDATA (EVIDENCE_ID) TABLESPACE DATA2;
ALTER TABLE EXPRESSIONDATA
ADD CONSTRAINT EXPDAT_EV_FK FOREIGN KEY (EVIDENCE_ID) REFERENCES EXPRESSIONEVIDENCE (EVIDENCE_ID)
ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE VALIDATE;
ALTER TABLE EXPRESSIONDATA
ADD CONSTRAINT EXPDAT_BIOENT_FK FOREIGN KEY (BIOENTITY_ID) REFERENCES LOCUSBIOENTITY (BIOENTITY_ID)
ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE VALIDATE;
GRANT SELECT ON EXPRESSIONDATA TO DBSELECT;
GRANT DELETE, INSERT, SELECT, UPDATE ON EXPRESSIONDATA TO CURATOR;
GRANT DELETE, INSERT, SELECT, UPDATE ON EXPRESSIONDATA TO AUXILIARY;
DROP SEQUENCE EXPDATA_SEQ;
CREATE SEQUENCE EXPDATA_SEQ NOCACHE;
