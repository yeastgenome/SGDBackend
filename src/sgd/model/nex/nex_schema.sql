...
RELATION_TYPE VARCHAR2(40) NOT NULL (RELATION_TYPE IN ('is a')),
DATE_CREATED DATE DEFAULT SYSDATE NOT NULL,
CREATED_BY VARCHAR2(12) DEFAULT SUBSTR(USER,1,12) NOT NULL,
...
SUBCLASS VARCHAR2(40) NOT NULL CHECK (SUBCLASS IN ('LOCUS', 'REFERENCE', 'STRAIN', 'FILE', 'DBENTITY')),
...
LOB (DELETED_ROW) store as BASICFILE (TABLESPACE LOB01
enable storage in row CHUNK 8K nocache nologging pctversion 10);
...
LOB (OLD_VALUE) store as BASICFILE (TABLESPACE LOB01
enable storage in row CHUNK 8K nocache nologging pctversion 10)
LOB (NEW_VALUE) store as BASICFILE (TABLESPACE LOB01
enable storage in row CHUNK 8K nocache nologging pctversion 10);
