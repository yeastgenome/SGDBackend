...
DROP TABLE LOCUS_CHANGE CASCADE CONSTRAINTS;
CREATE TABLE LOCUS_CHANGE (
...
DATE_CREATED DATE DEFAULT SYSDATE NOT NULL,
CREATED_BY VARCHAR2(12) DEFAULT SUBSTR(USER,1,12) NOT NULL,
CONSTRAINT LOCUS_CHANGE_PK PRIMARY KEY (HISTORY_ID),
CONSTRAINT LOCUS_CHANGE_UK UNIQUE (LOCUS_ID, CHANGE_TYPE, OLD_VALUE, NEW_VALUE, DATE_CHANGE_MADE) ) TABLESPACE DATA02;
CREATE INDEX LOCUSCHG_SOURCE_FK_INDEX ON LOCUS_CHANGE (SOURCE_ID) TABLESPACE DATA02;
ALTER TABLE LOCUS_CHANGE
ADD CONSTRAINT LOCUSCHG_SOURCE_FK FOREIGN KEY (SOURCE_ID) REFERENCES SOURCE (SOURCE_ID)
ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE VALIDATE;
ALTER TABLE LOCUS_CHANGE
ADD CONSTRAINT LOCUSCHG_LOCUS_FK FOREIGN KEY (LOCUS_ID) REFERENCES LOCUSDBENTITY (DBENTITY_ID)
ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE VALIDATE;
GRANT SELECT ON LOCUS_CHANGE TO DBSELECT;
GRANT DELETE, INSERT, SELECT, UPDATE ON LOCUS_CHANGE TO CURATOR;
GRANT DELETE, INSERT, SELECT, UPDATE ON LOCUS_CHANGE TO AUXILIARY;
...
CURATION_TAG VARCHAR2(40) NOT NULL CHECK (CURATION_TAG IN ('')),
