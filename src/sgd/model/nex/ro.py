...
class Ro(Base, EqualityByIDMixin, ToJsonMixin, UpdateWithJsonMixin):
    ...
    id = Column('relation_ontology_id', Integer, primary_key=True)
    source_id = Column('source_id', Integer, ForeignKey(Source.id))
    display_name = Column('display_name', String)
    format_name = Column('format_name', String)
    link = Column('obj_url', String)
    bud_id = Column('bud_id', Integer)
    ro_id = Column('ro_id', String)
    ...
    __eq_values__ = ['id', 'display_name', 'format_name', 'link', 'description', 'bud_id', 'date_created', 'created_by', 'ro_id']
    __eq_fks__ = [('source', Source, False),
                  ('urls', 'ro.RoUrl', True),
                  ('children', 'ro.RoRelation', True)]
    __id_values__ = ['id', 'format_name', 'ro_id']
...
class RoUrl(Base, EqualityByIDMixin, UpdateWithJsonMixin, ToJsonMixin):
    ...
    source_id = Column('source_id', Integer, ForeignKey(Source.id))
    bud_id = Column('bud_id', Integer)
    relation_ontology_id = Column('relation_ontology_id', Integer, ForeignKey(Ro.id, ondelete='CASCADE'))
    ...
    def create_or_find(cls, obj_json, session, parent_obj=None):
        ...
        if parent_obj is not None:
            newly_created_object.relation_ontology_id = parent_obj.id

        current_obj = session.query(cls)\
            .filter_by(relation_ontology_id=newly_created_object.relation_ontology_id)\
