...
def arch_literatureannotation_starter(bud_session_maker):
    ...
    for bud_obj in bud_session.query(LitguideFeat).all():
        ...
        if litguide.topic in ['Additional Literature', 'Omics', 'Primary Literature', 'Reviews']:
            continue
        ...
        if bud_obj.feature_id is not None:
...
if __name__ == '__main__':
    from src.sgd.convert import config
    basic_convert(config.BUD_HOST, config.NEX_HOST, arch_literatureannotation_starter, 'arch_literatureannotation', lambda x: (x['topic'], x['reference_id'], x.get('locus_id')))
