...
def reservedname_starter(bud_session_maker):
    ...
    for gene_reservation in bud_session.query(GeneReservation).filter_by(date_standardized=None).all():
        ...
        yield dict(name=gene_reservation.reserved_gene_name,
                   ...
                   reservation_date=str(gene_reservation.reservation_date),
                   expiration_date=str(gene_reservation.expiration_date),
                   source={'name': 'SGD'},
...
def convert(bud_db, nex_db):
    basic_convert(bud_db, nex_db, reservedname_starter, 'reservedname', lambda x: x['name'])
