...
def phenotypeannotation_cond_starter(bud_session_maker):
    ...
    chebi_name_to_id =  dict([(x.display_name, x.id) for x in nex_session.query(Chebi).all()])

    expt_property_id_to_exptprop_obj= dict([(x.id, x) for x in bud_session.query(ExperimentProperty).all()])

    expt_property_id_to_expt_id_list = {}
    for x in bud_session.query(Experiment_ExperimentProp).all():
        expt_id_list = []
        if x.experiment_property_id in expt_property_id_to_expt_id_list:
            expt_id_list = expt_property_id_to_expt_id_list[x.experiment_property_id]
        expt_id_list.append(x.experiment_id)
        expt_property_id_to_expt_id_list[x.experiment_property_id] = expt_id_list
    ...
    for x in bud_session.query(ExperimentProperty).all():
        ...
        if x.type in ['chebi_ontology', 'Chemical_pending']:
            ...
            chebi_id = chebi_name_to_id.get(x.value)
            if chebi_id is None:
                print "Chebi term:", x.value, " is not in chebi table." 
                continue
            ...
            expt_id_list = expt_property_id_to_expt_id_list.get(x.id)
    ...
    for line in f:
        ...
        if len(pieces) < 5:
            print "bad row: ", line 
            continue
        expt_id_list = expt_property_id_to_expt_id_list.get(int(pieces[0]))
        if expt_id_list is None:
