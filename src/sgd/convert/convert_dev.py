...
if __name__ == "__main__":   
    ...
    nex_session_maker = prepare_schema_connection(nex, config.NEX_DBTYPE, 'curator-dev-db.stanford.edu:1521', config.NEX_DBNAME, config.NEX_SCHEMA, config.NEX_DBUSER, config.NEX_DBPASS)
    ...
    from src.sgd.convert.from_bud.evelements import make_source_starter
    from src.sgd.model.nex.secondary import Source

    nex_session_maker().query(Source).all()
    ...
    #               [Json2Obj(Disambig),
    #                Obj2NexDB(nex_session_maker, lambda x: x.query(Disambig).filter(Disambig.class_type == 'EXPERIMENT'),
    #                          name='convert.from_bud.experiment.disambig',
    #                          delete_untouched=True,
    #                          commit=True)])
    #
    # # ------------------------------------------ Bioentity ------------------------------------------
    ...
    #
    # do_conversion(make_pathway_starter(bud_session_maker, nex_session_maker),
    #               [Json2Obj(Pathway),
    #                Obj2NexDB(nex_session_maker, lambda x: x.query(Pathway),
    #                          name='convert.from_bud.bioitem.pathway',
    #                          delete_untouched=True,
    #                          commit=True,
    ...
    # #               [Json2Obj(Disambig),
    # #                Obj2NexDB(nex_session_maker, lambda x: x.query(Disambig).filter(Disambig.class_type == 'BIOITEM').filter(Disambig.subclass_type == 'DATASET'),
    ...
    # #                Evidence2NexDB(nex_session_maker, lambda x: x.query(ArchiveLiteratureevidence),
    # #                               name='convert.from_bud.evidence.archive_literature',
    # #                               delete_untouched=True,
    # #                               commit_interval=1000,
    # #                               already_deleted=clean_up_orphans(nex_session_maker, ArchiveLiteratureevidence, Evidence, 'ARCH_LITERATURE')),
    # #                OutputTransformer(1000)])
    #
    ...
    # #                    Obj2NexDB(nex_session_maker, lambda x: x.query(Proteinsequenceevidence).filter(Proteinsequenceevidence.strain_id == strain_key_to_id[strain_key]),
    # #                              name='convert.from_bud.evidence.proteinsequence',
    # #                              delete_untouched=True,
    # #                              commit_interval=1000,
    # #                              already_deleted=clean_up_orphans(nex_session_maker, Proteinsequenceevidence, Evidence, 'PROTEINSEQUENCE')),
    # #                    OutputTransformer(1000)])
    # #
    ...
    #     make_bioitem_interaction_starter, make_bioentity_physinteraction_starter, make_bioentity_geninteraction_starter, \
    #     make_bioentity_expression_interaction_starter, make_bioentity_regulation_interaction_starter, make_bioconcept_count_starter
    # #
    ...
    #                                name='convert.from_backend.experiment',
    #                                commit_interval=100,
    #                                delete_untouched=True),
    ...
    #                                name='convert.from_backend.all_locusentries',
    #                                commit_interval=1000,
    #                                delete_untouched=True),
    #                OutputTransformer(1000)])
    #
    ...
    #               [Json2CorePerfDB(perf_session_maker, PerfBioconcept,
    #                                name='convert.from_backend.bioconcept',
    #                                delete_untouched=True,
    #                                commit=True),
    #                OutputTransformer(1000)])
    #
    ...
    #               [Json2CorePerfDB(perf_session_maker, PerfReference,
    #                                name='convert.from_backend.reference',
    #                                delete_untouched=True,
    #                                commit_interval=1000),
    #                OutputTransformer(1000)])
    #
    # do_conversion(make_backend_starter(nex_backend, 'all_authors', 1000),
    #               [Json2CorePerfDB(perf_session_maker, PerfAuthor,
    #                                name='convert.from_backend.author',
    #                                delete_untouched=True,
    #                                commit_interval=1000),
    #                OutputTransformer(1000)])
    #
    # do_conversion(make_backend_starter(nex_backend, 'all_bibentries', 1000),
    #               [Json2CorePerfDB(perf_session_maker, PerfBibentry,
    #                                name='convert.from_backend.all_bibentries',
    #                                commit_interval=1000,
    #                                delete_untouched=True),
    #                OutputTransformer(1000)])
    #
    ...
    # #                 OutputTransformer(100)])
    #
    # do_conversion(make_orphan_backend_starter(nex_backend, ['references_this_week', 'all_locus', 'snapshot', 'tag_list']),
