...
from src.sgd.model.nex.evidence import Domainevidence, Phosphorylationevidence, Proteinexperimentevidence, Posttranslationalevidence
...
# -------------------------------Details---------------------------------------
def get_posttranslational_evidence(locus_id):
    query = DBSession.query(Posttranslationalevidence)
    if locus_id is not None:
        query = query.filter_by(locus_id=locus_id)
...
def make_posttranslational_details(locus_id=None):
    if locus_id is None:
        return {'Error': 'No locus_id given.'}

    return '[' + ', '.join([x.json for x in sorted(get_posttranslational_evidence(locus_id=locus_id), key=lambda x: x.site_index) if x.json is not None]) + ']'
