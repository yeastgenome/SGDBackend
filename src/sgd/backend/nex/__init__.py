...
class SGDBackend(BackendInterface):
    ...
    def snapshot(self):
        from src.sgd.backend.nex.view_snapshot import get_snapshot
        return get_snapshot()
    ...
    def evidence(self, evidence_identifier, are_ids=False):
        from src.sgd.model.nex.evidence import Evidence
        return None if evidence_identifier is None else json.dumps(DBSession.query(Evidence).filter_by(id=evidence_identifier).first().to_json())
