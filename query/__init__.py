...
def get_reference(reference_name, print_query=False):
    ...
    try:
        ...
        query = orig_query.filter(Reference.primary_dbxref_id == reference_name)
        reference = query.first() 
    if print_query:
        print query
    return reference
...
def get_interaction_evidence(bioent_id=None, biorel_id=None, reference_id=None, print_query=False):
    ...
    if bioent_id is not None:
        ...
        interevidences = set(retrieve_in_chunks(biorel_ids, f))
    if biorel_id is not None:
        query = query.filter(Interevidence.biorel_id==biorel_id)
        interevidences = interevidences.intersection(set(query.all()))
    if reference_id is not None:
