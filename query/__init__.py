...
def get_biocon(biocon_name, biocon_type, print_query=False):
    ...
    '''
    query = session.query(with_polymorphic(Bioconcept, [biocon_type_to_class[biocon_type]])).filter(Bioconcept.biocon_type==biocon_type).filter(Bioconcept.official_name==biocon_name)
    biocon = query.first()
    if print_query:
        print query
    return biocon
...
def get_biocon_id(biocon_name, biocon_type, print_query=False):
    ...
    WHERE sprout.biocon.biocon_type = :biocon_type_1 AND sprout.biocon.name = :name_1
    '''
    query = session.query(Bioconcept).filter(Bioconcept.biocon_type==biocon_type).filter(Bioconcept.official_name==biocon_name)
    biocon = query.first()
    biocon_id = None
...
def get_bioent(bioent_name, print_query=False):
    ...
    '''
    query = session.query(with_polymorphic(Bioentity, [Gene])).filter(Bioentity.official_name==bioent_name)
    bioent = query.first()
    if print_query:
        print query
    return bioent
...
def get_bioent_id(bioent_name, print_query=False):
    ...
    WHERE sprout.bioent.name = :name_1
    '''
    query = session.query(Bioentity).filter(Bioentity.official_name==bioent_name)
    bioent = query.first()
    bioent_id = None
...
def get_biorel(biorel_name, biorel_type, print_query=False):
    ...
    '''
    query = session.query(BioentRelation).filter(BioentRelation.biorel_type==biorel_type).filter(BioentRelation.official_name==biorel_name)
    biorel = query.first()
    if print_query:
        print query
    return biorel
...
def get_reference(reference_name, print_query=False):
    ...
    '''
    reference = None
    orig_query = session.query(Reference).options(joinedload('author_references'), joinedload('author_references.author'), joinedload('abst'))
    try:
        float(reference_name)
        query = orig_query.filter(Reference.pubmed_id == reference_name)
        reference = query.first()
        if reference is None:
            query = orig_query.filter(Reference.id == reference_name)
            reference = query.first()
    except:
        query = orig_query.filter(Reference.dbxref_id == reference_name)
        reference = query.first() 
    if print_query:
        print query
    return reference
...
def get_reference_id(reference_name, print_query=False):
    ...
    '''
    reference = None
    orig_query = session.query(Reference)
    try:
        float(reference_name)
        query = orig_query.filter(Reference.pubmed_id == reference_name)
        reference = query.first()
...
def get_biorels(biorel_type, bioent_id, print_query=False):
    ...
    query1 = session.query(querycls).options(joinedload(querycls.source_bioent), joinedload(querycls.sink_bioent)).filter(querycls.biorel_type==biorel_type).filter(querycls.source_bioent_id == bioent_id)
    query2 = session.query(querycls).options(joinedload(querycls.source_bioent), joinedload(querycls.sink_bioent)).filter(querycls.biorel_type==biorel_type).filter(querycls.sink_bioent_id == bioent_id)
    biorels = set(query1.all())
    biorels.update(query2.all())
    if print_query:
        print query1
        print query2
    return biorels
...
def get_interactions(bioent_ids, print_query=False):
    ...
    query1 = session.query(querycls).filter(querycls.biorel_type=='INTERACTION').filter(querycls.source_bioent_id.in_(bioent_ids))
    query2 = session.query(querycls).filter(querycls.biorel_type=='INTERACTION').filter(querycls.sink_bioent_id.in_(bioent_ids))
    interactions = set(query1.all())
    interactions.update(query2.all())
    if print_query:
        print query1
        print query2
    return interactions
...
def get_phenotype_evidence(bioent_id=None, biocon_id=None, reference_id=None, print_query=False):
    '''
...
def get_interaction_evidence(bioent_id=None, biorel_id=None, reference_id=None, print_query=False):
    '''
    ...
    '''
    query = session.query(Interevidence).options(joinedload('reference'))
    interevidences = []
    if bioent_id is not None:
        biorel_ids = [biorel.id for biorel in get_biorels('INTERACTION', bioent_id)]
        def f(chunk_biorel_ids):
            new_query = query.filter(Interevidence.biorel_id.in_(chunk_biorel_ids))
            interevidences = new_query.all()
            if print_query:
                print new_query
            return interevidences
        interevidences = retrieve_in_chunks(biorel_ids, f)
    if biorel_id is not None:
        query = query.filter(Interevidence.biorel_id==biorel_id)
        interevidences = interevidences.intersection(query.all())
    if reference_id is not None:
        query = query.filter(Interevidence.reference_id==reference_id)
        interevidences = interevidences.intersection(query.all())
    if print_query:
        print_query
    return interevidences
